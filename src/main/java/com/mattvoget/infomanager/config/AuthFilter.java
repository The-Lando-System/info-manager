    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        try {
            ...
            if ((user.getRole() != Role.ADMIN) || (!StringUtils.equals(user.getToken().getAccessToken(),headers.get("x-access-token")))){
                ...
                InfoManagerException ime = new InfoManagerException("You do not have permissions to access this resource", IllegalAccessError.class.getSimpleName(), HttpStatus.UNAUTHORIZED);
                ObjectMapper mapper = new ObjectMapper();

                servletResponse.setContentType("application/json");
                servletResponse.setCharacterEncoding("UTF-8");

                String jsonResponse = mapper.writeValueAsString(ime);
